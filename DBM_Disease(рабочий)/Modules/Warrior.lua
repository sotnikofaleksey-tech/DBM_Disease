-- DBM_Disease Warrior Module
-- IDENTICAL TO WORKING MODULES

-- Module registration
DBM_Disease_Modules = DBM_Disease_Modules or {}
DBM_Disease_Modules.Warrior = {
    name = "Warrior",
    class = "WARRIOR",
    
    Initialize = function()
        if DBM_Disease and DBM_Disease.Debug then
            DBM_Disease.Debug("=== Loading Warrior Rotation Module ===")
        end
        
        -- Проверка: работает только для класса WARRIOR (воин)
        ...
        -- Функция поиска заклинания (как в рабочем модуле монаха)
        function _Get(Q)
        ...
        local function T()
            ...
            -- Для всех специализаций воина (1-3)
            if (U==1 or U==2 or U==3) and L=="WARRIOR" then 
                -- Пробуем найти Hekili в D1 фреймах
                local hekiliTexture = nil
                
                -- Проверяем D1 фреймы (как в рабочем модуле)
                for i=1,5 do
                ...
                if GetCVar("VadimRogueRM")=="1" then 
                    if _G[g]._texture then
                        ...
                        -- ФОЛБЭК РОТАЦИЯ - ОСНОВНЫЕ ЗАКЛИНАНИЯ ВОИНА
                        local fallbackSpells = {
                            {id = 78, name = "Удар героя"},         -- Heroic Strike
                            {id = 100, name = "Рывок"},             -- Charge
                            {id = 6552, name = "Удар"},             -- Pummel
                            {id = 34428, name = "Разрубание"},      -- Victorious Strike
                            {id = 6572, name = "Отражение"},        -- Revenge
                            {id = 23922, name = "Щитовая блокировка"}, -- Shield Block
                            {id = 46924, name = "Удар грома"},      -- Bladestorm
                            {id = 85288, name = "Сокрушение"},      -- Colossus Smash
                        }
                        
                        for _, spell in ipairs(fallbackSpells) do
                            if N(spell.id) == 0 then
                                -- Особые проверки для определенных заклинаний
                                if spell.id == 100 then -- Рывок (курсор)
                                    RunMacroText("/cast [@cursor] " .. spell.name)
                                else
        ...
        if DBM_Disease and DBM_Disease.Debug then
            DBM_Disease.Debug("Warrior rotation module loaded successfully")
        end
        return true
    end
}
...
if DBM_Disease and DBM_Disease.RegisterModule then
    DBM_Disease.RegisterModule("WARRIOR", DBM_Disease_Modules.Warrior)
end