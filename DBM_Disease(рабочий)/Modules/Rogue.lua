-- DBM_Disease Rogue Module
-- IDENTICAL TO WORKING MODULES

-- Module registration
DBM_Disease_Modules = DBM_Disease_Modules or {}
DBM_Disease_Modules.Rogue = {
    name = "Rogue",
    class = "ROGUE",
    
    Initialize = function()
        if DBM_Disease and DBM_Disease.Debug then
            DBM_Disease.Debug("=== Loading Rogue Rotation Module ===")
        end
        
        -- Проверка: работает только для класса ROGUE
        ...
        local function T()
            ...
            -- Для всех специализаций разбойника (1-3)
            if (U==1 or U==2 or U==3) and L=="ROGUE" then 
                ...
                if GetCVar("VadimRogueRM")=="1" then 
                    if _G[g]._texture then
                        ...
                        -- ФОЛБЭК РОТАЦИЯ - ОСНОВНЫЕ ЗАКЛИНАНИЯ РАЗБОЙНИКА
                        local fallbackSpells = {
                            {id = 1752, name = "Коварный удар"},     -- Sinister Strike
                            {id = 196819, name = "Кровоизлияние"},   -- Eviscerate
                            {id = 53, name = "Коварный удар"},       -- Backstab
                            {id = 1943, name = "Кровопускание"},     -- Rupture
                            {id = 703, name = "Смертельный яд"},     -- Garrote
                            {id = 8676, name = "Амбуша"},            -- Ambush
                            {id = 315584, name = "Между глаз"},      -- Between the Eyes
                            {id = 185565, name = "Разбойничий выстрел"}, -- Pistol Shot
                        }
        ...
        if DBM_Disease and DBM_Disease.Debug then
            DBM_Disease.Debug("Rogue rotation module loaded successfully")
        end
        return true
    end
}
...
if DBM_Disease and DBM_Disease.RegisterModule then
    DBM_Disease.RegisterModule("ROGUE", DBM_Disease_Modules.Rogue)
end