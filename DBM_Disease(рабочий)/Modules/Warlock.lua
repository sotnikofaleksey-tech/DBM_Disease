-- DBM_Disease Warlock Module
-- IDENTICAL TO WORKING MODULES

-- Module registration
DBM_Disease_Modules = DBM_Disease_Modules or {}
DBM_Disease_Modules.Warlock = {
    name = "Warlock",
    class = "WARLOCK",
    
    Initialize = function()
        if DBM_Disease and DBM_Disease.Debug then
            DBM_Disease.Debug("=== Loading Warlock Rotation Module ===")
        end
        
        -- Проверка: работает только для класса WARLOCK
        ...
        local function T()
            ...
            -- Для всех специализаций чернокнижника (1-3)
            if (U==1 or U==2 or U==3) and L=="WARLOCK" then 
                ...
                if GetCVar("VadimRogueRM")=="1" then 
                    if _G[g]._texture then
                        ...
                        -- ФОЛБЭК РОТАЦИЯ - ОСНОВНЫЕ ЗАКЛИНАНИЯ ЧЕРНОКНИЖНИКА
                        local fallbackSpells = {
                            {id = 686, name = "Стрела Тьмы"},         -- Shadow Bolt
                            {id = 172, name = "Порча"},              -- Corruption
                            {id = 348, name = "Жертвенный огонь"},   -- Immolate
                            {id = 29722, name = "Испепеление"},      -- Incinerate
                            {id = 17877, name = "Ожог Тьмы"},        -- Shadowburn
                            {id = 980, name = "Агония"},             -- Agony
                            {id = 30108, name = "Нестабильное колдовство"}, -- Unstable Affliction
                        }
        ...
        if DBM_Disease and DBM_Disease.Debug then
            DBM_Disease.Debug("Warlock rotation module loaded successfully")
        end
        return true
    end
}
...
if DBM_Disease and DBM_Disease.RegisterModule then
    DBM_Disease.RegisterModule("WARLOCK", DBM_Disease_Modules.Warlock)
end