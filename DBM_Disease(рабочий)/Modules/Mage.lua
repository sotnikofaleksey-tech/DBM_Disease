-- DBM_Disease Mage Module
-- IDENTICAL TO WORKING MODULES

-- Module registration
DBM_Disease_Modules = DBM_Disease_Modules or {}
DBM_Disease_Modules.Mage = {
    name = "Mage",
    class = "MAGE",
    
    Initialize = function()
        if DBM_Disease and DBM_Disease.Debug then
            DBM_Disease.Debug("=== Loading Mage Rotation Module ===")
        end
        
        -- Проверка: работает только для класса MAGE (маг)
        ...
        local function T()
            ...
            -- Для всех специализаций мага (1-3)
            if (U==1 or U==2 or U==3) and L=="MAGE" then 
                ...
                if GetCVar("VadimRogueRM")=="1" then 
                    if _G[g]._texture then
                        ...
                        -- ФОЛБЭК РОТАЦИЯ - ОСНОВНЫЕ ЗАКЛИНАНИЯ МАГА
                        local fallbackSpells = {
                            {id = 133, name = "Огненный шар"},       -- Fireball
                            {id = 116, name = "Ледяная стрела"},     -- Frostbolt
                            {id = 44425, name = "Живая бомба"},      -- Living Bomb
                            {id = 30455, name = "Ледяная стрела"},   -- Ice Lance
                            {id = 44614, name = "Вспышка огня"},     -- Flamestrike
                            {id = 30451, name = "Ледяная глыба"},    -- Arcane Blast
                            {id = 10, name = "Ледяная буря"},        -- Blizzard
                            {id = 11366, name = "Пироклазм"},        -- Pyroblast
                        }
                        
                        for _, spell in ipairs(fallbackSpells) do
                            if N(spell.id) == 0 then
                                -- Особые проверки для определенных заклинаний
                                if spell.id == 10 or spell.id == 44614 then -- Ледяная буря, Вспышка огня (курсор)
        ...
        if DBM_Disease and DBM_Disease.Debug then
            DBM_Disease.Debug("Mage rotation module loaded successfully")
        end
        return true
    end
}
...
if DBM_Disease and DBM_Disease.RegisterModule then
    DBM_Disease.RegisterModule("MAGE", DBM_Disease_Modules.Mage)
end