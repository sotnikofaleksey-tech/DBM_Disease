-- DBM_Disease Death Knight Module
-- IDENTICAL TO WORKING MODULES

-- Module registration
DBM_Disease_Modules = DBM_Disease_Modules or {}
DBM_Disease_Modules.DeathKnight = {
    name = "DeathKnight",
    class = "DEATHKNIGHT",
    
    Initialize = function()
        if DBM_Disease and DBM_Disease.Debug then
            DBM_Disease.Debug("=== Loading Death Knight Rotation Module ===")
        end
        
        -- Проверка: работает только для класса DEATHKNIGHT
        ...
        local function T()
            ...
            -- Для всех специализаций рыцаря смерти (1-3)
            if (U==1 or U==2 or U==3) and L=="DEATHKNIGHT" then 
                ...
                if GetCVar("VadimRogueRM")=="1" then 
                    if _G[g]._texture then
                        ...
                        -- ФОЛБЭК РОТАЦИЯ - ОСНОВНЫЕ ЗАКЛИНАНИЯ РЫЦАРЯ СМЕРТИ
                        local fallbackSpells = {
                            {id = 49998, name = "Удар смерти"},      -- Death Strike
                            {id = 47541, name = "Ликвидация"},       -- Death Coil
                            {id = 85948, name = "Потрошение"},       -- Festering Strike
                            {id = 55090, name = "Чумной удар"},      -- Scourge Strike
                            {id = 49184, name = "Ледяное прикосновение"}, -- Howling Blast
                            {id = 49020, name = "Удар в спину"},     -- Obliterate
                            {id = 45462, name = "Удар чумы"},        -- Plague Strike
                            {id = 50842, name = "Кипящая кровь"},    -- Blood Boil
                        }
        ...
        if DBM_Disease and DBM_Disease.Debug then
            DBM_Disease.Debug("Death Knight rotation module loaded successfully")
        end
        return true
    end
}
...
if DBM_Disease and DBM_Disease.RegisterModule then
    DBM_Disease.RegisterModule("DEATHKNIGHT", DBM_Disease_Modules.DeathKnight)
end