-- DBM_Disease Hunter Module
-- IDENTICAL TO WORKING MODULES

-- Module registration
DBM_Disease_Modules = DBM_Disease_Modules or {}
DBM_Disease_Modules.Hunter = {
    name = "Hunter",
    class = "HUNTER",
    
    Initialize = function()
        if DBM_Disease and DBM_Disease.Debug then
            DBM_Disease.Debug("=== Loading Hunter Rotation Module ===")
        end
        
        -- Проверка: работает только для класса HUNTER
        ...
        local function T()
            ...
            -- Для всех специализаций охотника (1-3)
            if (U==1 or U==2 or U==3) and L=="HUNTER" then 
                ...
                if GetCVar("VadimRogueRM")=="1" then 
                    if _G[g]._texture then
                        ...
                        -- ФОЛБЭК РОТАЦИЯ - ОСНОВНЫЕ ЗАКЛИНАНИЯ ОХОТНИКА
                        local fallbackSpells = {
                            {id = 56641, name = "Стремительный выстрел"},  -- Steady Shot
                            {id = 34026, name = "Убийственный выстрел"},   -- Kill Shot
                            {id = 193455, name = "Разрывающий выстрел"},   -- Cobra Shot
                            {id = 19434, name = "Прицельный выстрел"},     -- Aimed Shot
                            {id = 53351, name = "Метательный нож"},        -- Kill Command
                            {id = 19574, name = "Ярость зверя"},           -- Bestial Wrath
                            {id = 3044, name = "Огненный выстрел"},        -- Arcane Shot
                        }
        ...
        if DBM_Disease and DBM_Disease.Debug then
            DBM_Disease.Debug("Hunter rotation module loaded successfully")
        end
        return true
    end
}
...
if DBM_Disease and DBM_Disease.RegisterModule then
    DBM_Disease.RegisterModule("HUNTER", DBM_Disease_Modules.Hunter)
end