-- DBM_Disease Paladin Module
-- IDENTICAL TO WORKING MODULES

-- Module registration
DBM_Disease_Modules = DBM_Disease_Modules or {}
DBM_Disease_Modules.Paladin = {
    name = "Paladin",
    class = "PALADIN",
    
    Initialize = function()
        if DBM_Disease and DBM_Disease.Debug then
            DBM_Disease.Debug("=== Loading Paladin Rotation Module ===")
        end
        
        -- Проверка: работает только для класса PALADIN
        ...
        local function T()
            ...
            -- Для всех специализаций паладина (1-3)
            if (U==1 or U==2 or U==3) and L=="PALADIN" then 
                ...
                if GetCVar("VadimRogueRM")=="1" then 
                    if _G[g]._texture then
                        ...
                        -- ФОЛБЭК РОТАЦИЯ - ОСНОВНЫЕ ЗАКЛИНАНИЯ ПАЛАДИНА
                        local fallbackSpells = {
                            {id = 35395, name = "Удар воина Света"},  -- Crusader Strike
                            {id = 20271, name = "Суждение"},          -- Judgment
                            {id = 85256, name = "Молот праведника"},  -- Templar's Verdict
                            {id = 24275, name = "Молот гнева"},       -- Hammer of Wrath
                            {id = 53595, name = "Молот праведного гнева"}, -- Hammer of the Righteous
                            {id = 31935, name = "Щит мстителя"},      -- Avenger's Shield
                            {id = 53600, name = "Щит небес"},         -- Shield of the Righteous
                        }
        ...
        if DBM_Disease and DBM_Disease.Debug then
            DBM_Disease.Debug("Paladin rotation module loaded successfully")
        end
        return true
    end
}
...
if DBM_Disease and DBM_Disease.RegisterModule then
    DBM_Disease.RegisterModule("PALADIN", DBM_Disease_Modules.Paladin)
end